...
const Header = () => {
	const [settingsForSearching, setSettingsForSearching] = useState(null); 
	const handleOnChange = (event) => {
		axios
			...
			.then((response) => {
				setSettingsForSearching(response.data);
			});
	}
		...
		<div className="mvx-header-wapper">
			...
			<div className="mvx-header-nav-right-section">
				<div className="mvx-header-search-section">
					...
					/>
					{
						settingsForSearching?.length > 0 &&
						<div className="mvx-search-content">
							{
								settingsForSearching.map((data) => (
									<div className="mvx-header-search-content-wrapper">
										<a onClick={(e) => window.location.href = data.link}>
											...
											<p className="mvx-header-search-contetnt-desc" >
												{data.desc}
											</p>
											<p className="mvx-header-search-contetnt-desc" >
												{data.details}
