...
import CustomInput from '../components/admin/CustomInputs';
...
const DynamicForm = (props) => {
    ...
    useEffect(() => {
        setSetting(props.globalSettings.current[props.modelName]);
    }, [props.modelName]);
    ...
    useEffect(() => {
        if (settingChanged.current) {
            ...
            const { globalSettings, modelName } = props;
            axios({
                method: props.method,
                url: appLocalizer.apiUrl + '/' + props.apiurl,
                data: {
                    model: setting,
                    modulename: props.modelName,
                    vendor_id: props.vendorId || '',
                    announcement_id: props.announcementId || '',
                    knowladgebase_id: props.knowladgebaseId || '',
                },
            }).then((response) => {
                globalSettings.current = { ...globalSettings.current, [modelName]: setting };
                setSuccessMsg(response.data.error);
                setTimeout(() => {
                    setSuccessMsg('');
                }, 2000);
    ...
    const handleChange = (event, key, type = 'single', fromType = 'simple', arrayValue = []) => {
        ...
        if (type === 'single') {
            if (fromType === 'simple') {
                ...
                setSetting((setting) => {
                    return { ...setting, [key]: event.join(',') }
                });
            } else if (fromType === 'select') {
                setSetting((setting) => {
                    return { ...setting, [key]: arrayValue[event.index]}
                });
            } else if (fromType === 'multi-select') {
                setSetting((setting) => {
                    return { ...setting, [key]: event }
                });
    ...
    const handleMultiNumberChange = (e, key, optionKey, index) => {
        ...
        setSetting((setting) => {
            ...
            return { ...setting, [key]: mulipleOptions };
        });
    }
    ...
    const handlMultiSelectDeselectChange = (e, m) => {
        ...
        if (setting[m.key].length > 0) {
            ...
            setSetting((setting) => {
                return { ...setting, [m.key]: complete_option_value };
            });
		}
    }
    ...
    const runUploader = (key) => {
		...
		frame.on('select', function () {
            ...
            setSetting((setting) => {
                return { ...setting, [key]: attachment.url}
            });
		});
    ...
    const renderForm = () => {
        ...
        return model.map((inputFild, index) => {
